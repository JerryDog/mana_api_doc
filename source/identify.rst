...
|  资源            |  操作                        |  HTTP方法  |  URI路径                       |
+==================+==============================+============+================================+
|  Tokens (令牌)   |  :ref:`keystone-get-token`  |  POST      |  /mana_api/tokens              |
+------------------+------------------------------+------------+--------------------------------+
...
::
    ...
    curl -i -s \
         -X POST http://api.scloudm.com/mana_api/tokens \
...
::
   ...
   {
     "msg": "Expecting to find password in passwordCredentials. The server could not comply with the request since it is either malformed or otherwise incorrect. The client is assumed to be in error.",
     "code": 400,
     "reason": "Unauthorized"
   }